...
const createPdf = async (req, res) => {
    ...
    const Ispolnitel = req.body.ispolnitel; // Предполагаем, что текст передается в теле запроса как { text: "Текст здесь" }
    const Date = req.body.date;
    const Address = req.body.address;
    const actAddress = req.body.act_address;
    const sosedAddres = req.body.sosed_address;
    const hisName = req.body.his_name;
    const gradeTrans = req.body.grade_transp;
    const School = req.body.school;
    const torCulCenter = req.body.tor_cul_center;
    const GradeEcologZone = req.body.grade_eco;
    const HomeLocation = req.body.home_loc;
    const ChildZone = req.body.child_zone;
    const CleanTerr = req.body.clean_terr;
    const Nasazh = req.body.nasazh;
    const MaterSten = req.body.mater_sten;
    const SostKrovl = req.body.sost_krovl;
    const VneshObshSten = req.body.vnesh_obsh_sten;
    const BlizhSpesUdob = req.body.blizh_spec_udob;
    const SostVneshObsh = req.body.sost_vnesh_obsh;
    const Santeh = req.body.santeh;
    const Krovlya = req.body.krovlya;
    const Podezd = req.body.podezd;
    const Gas = req.body.gas;
    const HolVod = req.body.hol_vod;
    const Podval = req.body.podval;
    const GorVod = req.body.gor_vod;
    const Cherdak = req.body.cherdak;
    const Canal = req.body.canal;
    const KolKom = req.body.kol_kom;
    const Etazh = req.body.etazh;
    const KolEtazh =req.body.kol_etazh;
    const Year = req.body.year;
    const Plosh = req.body.plosh;
    const ZhilPlosh = req.body.zhil_plosh
    const KuhPLosh = req.body.kuh_plosh
    ...
    doc
        ...
        .fillColor('#50575E')
        .font('Regular')
        .text(GradeEcologZone, 330, 500);

        doc
        .fontSize(20)
        .fillColor('#09C18A')
        .font('SemiBold')
        .text('Характеристика дома', 34, 530);
        ...
        .fillColor('#50575E')
        .font('Regular')
        .text(GradeEcologZone, 35, 440);

        doc
        .fontSize(12) 
        .fillColor('#94A3B8')
        .font('Regular')
        .text('Гол последнего кап. ремонта', 330, 420, {
        });
